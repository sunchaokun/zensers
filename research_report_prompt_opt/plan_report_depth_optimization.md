...
| R21 | 优化rubric定义 | -11.0 (81.5→70.5) | rubric调整导致退化 |

### 关键经验
1. **适度的prompt引导有效**：R13（规划阶段）和R15（前瞻性分析）通过适度引导LLM思考流程提升了质量
2. **过度约束导致退化**：所有添加新约束的尝试都导致质量下降
3. **审查维度增加适得其反**：在review阶段增加分析深度维度反而降低了报告质量
4. **截断放宽无显著效果**：global_reviewer的截断从400→1200没有带来质量提升
5. **修改现有要求也可能退化**：R19尝试修改现有结构要求使其更具体，但导致严重退化（-24.0）
6. **rubric调整也可能退化**：R21尝试优化rubric定义，但导致退化（-11.0），说明rubric已经是LLM理解的最佳状态

### 当前锁定状态
- **R13锁定**：规划阶段+框架评估输出（80.2/100）
- **R15锁定**：前瞻性分析要求（81.5/100）
- **R14/R16-R19/R21已revert**：保持R15锁定分81.5/100
