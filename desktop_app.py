...
def start_backend():
    ...
    # Clean up stuck processes on backend port
    _kill_port(BACKEND_PORT)
    
    log_dir = PROJECT_ROOT / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    err_log = open(log_dir / "uvicorn_stderr.log", "a", encoding="utf-8")
    out_log = open(log_dir / "uvicorn_stdout.log", "a", encoding="utf-8")
...
def start_frontend():
    ...
    # Kill any process on the fixed port to ensure clean start
    ...
    if check_pm2_running():
        ...
        print("Starting frontend service (via PM2)...")
...
class Api:
    ...
    def save_file(self, default_filename: str = "document.docx", file_type: str = "docx"):
        ...
        try:
            ...
            # File type filters - pywebview expects string format: 'Description (*.ext)'
            file_types_map = {
    ...
    def download_and_save(self, url: str, default_filename: str = "document.docx", file_type: str = "docx"):
        """
        Open save dialog, download file from URL, and save to selected location.
        ...
        try:
            ...
            # 3. Save to temporary file first, then copy (atomic write)
            with tempfile.NamedTemporaryFile(delete=False, suffix=f".{file_type}") as tmp:
                tmp.write(response.content)
                tmp_path = tmp.name
            
            # 4. Copy to user-selected location
...
def main():
    ...
    try:
        ...
        print("Please run: pip install webview")
        input("Press Enter to exit...")
        return
    ...
    # Add ?desktop=1 flag for React components to show Apple-style title bar
    ...
    try:
        # Create frameless window - title bar rendered by React built-in component
        _window = webview.create_window(
            ...
            min_size=(900, 600),
            frameless=True,
            easy_drag=False,   # Disable global drag, handled by CSS -webkit-app-region
            resizable=True,
            text_select=True,
            confirm_close=True,
        ...
        # Stop frontend service when closing window
        print()
