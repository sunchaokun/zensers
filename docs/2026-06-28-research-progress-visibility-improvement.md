...
> 日期: 2026-06-28（修订: 2026-06-29，审查修正: 2026-06-29，最终修订: 2026-06-30）
> 状态: **P0 + P1 + P2 已全部实现。所有未完成项已于 2026-06-30 修复**
> **注意**: GenericAgent 已添加 `_report_progress` 方法，18 处调用正常运行，25 个 tests 全部通过。
...
| SSE 类型扩展 | `api.ts` | ✅ | |
| AgentMessage 合并更新 | `ChatMessage.tsx` + `useChatStore.ts` | ✅ 已完成 | analyzing/searching/writing 同 agent_id 更新，completed/error 追加 |
| agent_message 频率节流 | `session_streamer.py` | ✅ | 200ms 节流窗口 |
| ProgressPanel 死代码清理 | `ProgressPanel.tsx` | ✅ 已清理 | 从 index.ts 移除导出 |
...
`_report_progress()` 方法已在 `BaseAgent`（`src/core/agents/base.py:231`）、`FixedAgent`（`src/agents/fixed_agents/base_fixed_agent.py:140`）和 `GenericAgent`（`generic_agent.py:178`）中实现。18 处调用分布在 research/quality-check/analysis/calibration/synthesis/fallback 各路径中，**已正常运行**（25 tests 通过）。
...
**行号说明**: 以下行号基于代码审查时的 generic_agent.py 版本，实际行号可能因后续改动偏移。

| Agent | 现有调用点 | 当前状态 | 覆盖的关键步骤 |
|-------|-----------|---------|---------------|
| GenericAgent (research) | 5 行 | ✅ 已实现 | 入口 + DB 汇总 + 搜索入口 + 搜索汇总 + news 补充 |
| GenericAgent (quality-check) | 4 行 | ✅ 已实现 | 入口 + 验证结果 + 冲突解决 + 重收集 |
| GenericAgent (analysis) | 5 行 | ✅ 已实现 | 入口 + 降级搜索 + 缺口检测 + 补充修订 + 自评 |
| GenericAgent (calibration) | 1 行 | ✅ 已实现 | 校准入口 |
| GenericAgent (synthesis) | 2 行 | ✅ 已实现 | LLM 调用前（enrichment + 无 enrichment 分支） |
| GenericAgent（fallback） | 1 行 | ✅ 已实现 | 搜索完成 |
| ReportGenerationAgent | 1 行 | ✅ 已实现 | 报告生成入口 |
| QualityCheckAgent | 1 行 | ✅ 已实现 | 质量检查入口 |
| DataCollectionAgent | 1 行 | ✅ 已实现 | 数据收集入口 |
...
| `src/core/agents/generic_agent.py` | 修改 | ~55 行 | ✅ |
...
| `web/src/components/chat/ProgressPanel.tsx` | 清理 | 从 index.ts 移除导出 | ✅ |
...
12. ~~**GenericAgent 不继承 BaseAgent（关键）**~~ — **已修复**。GenericAgent 已添加 `_report_progress` 方法（generic_agent.py:178），25 tests 通过。历史背景：GenericAgent 使用 mixin 组合不继承 BaseAgent，修复前 18 处调用全部抛 AttributeError。详见 §9.5 和 `docs/fix-generic-agent-report-progress.md`。
...
| P2-1 | Agent 内部 `_report_progress` 调用 | `generic_agent.py` + 各 FixedAgent | 25 tests ✅（含 4 GenericAgent） | ✅ **全部修复**（方法定义 + 18 处调用） |
...
| 改动 | 文件 | 说明 | 严重性 |
|------|------|------|--------|
| GenericAgent `_report_progress` 缺失 | `generic_agent.py` | ✅ **已修复**（方法已添加，4 tests 通过，§9.5） | 🔴 CRITICAL |
| Agent 内部进度细化（新增调用） | `generic_agent.py` | ✅ **已实现**（18 处调用覆盖 §6.3 全部进度点） | 🔴 阻塞 |
| ProgressPanel 死代码清理 | `ProgressPanel.tsx` | ✅ **已清理**（从 `index.ts` 移除导出，文件保留供参考） | 🟡 低 |
| AgentMessage 非 heartbeat 合并 | `ChatPanel.tsx` + `useChatStore.ts` | ✅ **已实现**（analyzing/searching/writing 同 agent_id 消息更新而非追加） | 🟢 低 |
...
| `tests/unit/test_generic_agent_report_progress.py` | 4 | ✅ |
...
### 9.5 ⚠️ 严重缺陷修复：GenericAgent `_report_progress` 实现

**发现过程**: 代码审查发现 `GenericAgent` 中现有 **18 处** `self._report_progress()` 调用，但 `GenericAgent` 不继承 `BaseAgent`，继承链 `StateManagementMixin + CommunicationMixin` 中均未定义 `_report_progress`。

**修复方案**: 详见独立设计文档 [`docs/fix-generic-agent-report-progress.md`](fix-generic-agent-report-progress.md)

**修复实施（方案 A）**:
1. 在 `GenericAgent.__init__` 之后添加 `_report_progress` 方法定义（L178）
2. 方法实现与 `BaseAgent._report_progress` 一致（`SessionStreamer.push_agent_message` + `try/except`）
3. 新增 `test_generic_agent_report_progress.py`（4 tests）覆盖全部分支

**修复结果**: ✅ **25 tests 全部通过**（含新增 4 个 GenericAgent tests）。18 处调用正常运行，不再抛 AttributeError。
