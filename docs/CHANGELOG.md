...
6 bugs identified and fixed that caused the frontend to lose chat mode state during async background search:

- **Fix 1**: `onChatResponse` SSE handler now processes `action`/`mode` fields (previously only `sendMessage` sync path did), adds `streamingDoneRef` re-entry guard and timestamp deduplication
- **Fix 2**: Backend `_continue_tool_chain_body` now includes `mode`/`step` in `response_data`; `ProgressStreamer` and `SessionStreamer` propagate these fields through SSE and persistence
- **Fix 3**: `useResearchStore` subscription refined — `status` change only updates progress fields, no longer overwrites `currentStep`/`framework`/`suggestions`
- **Fix 4**: `streamingDoneRef.current = true` added to `onChatResponse` else branch (was missing, causing replay protection failure)
- **Fix 5**: `enter_framework` action now handled in `onChatResponse` with API compensation (fetch framework detail)
- **Fix 6**: Backend `_continue_tool_chain_body` now handles `enter_framework` action — syncs state machine, calls `_enter_framework_mode`, sets `mode=framework`/`step=0` in response

Frontend type `ChatResponseData` extended with optional `mode` and `step` fields.
...
### System Architecture Design & Version Sync

- Added comprehensive system architecture design document (`docs/system_architecture_design.md`) covering 16 chapters: session management, intent analysis, intelligent routing, data collection, data analysis, report generation, knowledge management, quality assurance, skill plugins, storage, API, configuration, frontend, and cross-cutting concerns.
- Consolidated version source to `pyproject.toml` as single source of truth (removed redundant VERSION file and hardcoded `__version__` in version.py).

---

## [1.4.1] - 2026-06-29

### Agent Progress Reporting & LLM Routing Refactor

- Integrated agent progress reporting into research pipeline.
- Refactored LLM as intrinsic agent capability, not a skill.
